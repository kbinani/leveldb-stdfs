...
bool leveldb::ZstdCompressor::decompress(const char* input, size_t length,
                                         ::std::string& output) const {
  // extend the buffer to contain the worst case. Worst case is that input
  ...
  while (true) {
    bufsize *= 10;  // assume that the compression is compressing worse than
                    // 10%.
    // TODO use streams to decompress piece by piece, this is pretty wasteful
    // for memory & re-decompressions
    output.resize(originalSize + bufsize);

    auto sz = ZSTD_decompress((void*)(output.data() + originalSize), bufsize,
                              input, length);
    if (!ZSTD_isError(sz)) {
      output.resize(sz + originalSize);
      break;
